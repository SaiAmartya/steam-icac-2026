...
## 2. Full-clip three-way comparison demos
...
python scripts/compare_clip.py --clip clip_14 --crf 22 --no-sigmoid --show

# 5-panel including ours_sigmoid (the prior approach)
python scripts/compare_clip.py --clip clip_14 --crf 22 --show
...
```bash
python scripts/compare_clip.py --clip clip_02 --crf 22 --no-sigmoid --show
```
...
python scripts/compare_clip.py --clip clip_04 --crf 22 --no-sigmoid --show

# CRF 34 — show the old failure mode, then explain why we recommend CRF 22 for stationary scenes
python scripts/compare_clip.py --clip clip_04 --crf 34 --no-sigmoid --show
```
...
python scripts/compare_clip.py --clip clip_13 --crf 22 --no-saliency --no-sigmoid --show
```
...
### 3a. The full 5-panel comparison from your laptop camera

```bash
# Default: 2-second capture, CRF 28, both ours_sigmoid + ours_bgfg
python scripts/photo_demo.py --codec both --saliency yolo+spectral --show

# 5-second capture for a longer demo with movement
python scripts/photo_demo.py --codec both --duration 5 --saliency yolo+spectral --show

# Only bg/fg (cleaner panel for the headline pitch)
python scripts/photo_demo.py --codec bgfg --duration 5 --saliency yolo+spectral --show

# Higher quality
python scripts/photo_demo.py --codec both --crf 22 --duration 5 --saliency yolo+spectral --show
```
...
5-panel comparison for every clip × frame × CRF. Recommended showcase pages:
...
the prior sigmoid pipeline on the Pareto frontier.
...
| "How is this different from your old approach?" | `python scripts/compare_clip.py --clip clip_04 --crf 28 --show` (5-panel includes ours_sigmoid for direct comparison) |
...
| `results/comparisons/` | Single-clip 4/5-panel demo videos (compare_clip.py output) |
...
python scripts/compare_clip.py --clip clip_14 --crf 22 --no-sigmoid --out results/comparisons/headline_fall.mp4
python scripts/compare_clip.py --clip clip_02 --crf 22 --no-sigmoid --out results/comparisons/headline_grab.mp4
python scripts/compare_clip.py --clip clip_04 --crf 22 --no-sigmoid --out results/comparisons/headline_pool.mp4
